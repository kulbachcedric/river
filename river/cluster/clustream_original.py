...
class CluStreamOriginal(base.Clusterer):
    """CluStream (using traditional implementation of cluster features)
    ...
    * Check whether `p` fits (memory) into the closest micro-cluster:
    ...
    This implementation reflects the original version of `CluStream`, with the use of
    cluster features
    ...
    >>> clustream_original = cluster.CluStreamOriginal(
    ...     n_macro_clusters=3,
    ...
    ...     clustream_original = clustream_original.learn_one(x)

    >>> clustream_original.predict_one({0: 1, 1: 1})
    1

    >>> clustream_original.predict_one({0: -4, 1: 3})
    2

    >>> clustream_original.predict_one({0: 4, 1: 3.5})
    0
...
class CluStreamMicroCluster(base.Base):
    ...
    def __init__(
        ...
        if x is not None and w is not None:
            ...
            for key in x.keys():
                self.linear_sum[key] = x[key] * w
                self.squared_sum[key] = x[key] * x[key] * w
            self.linear_sum_timestamp = timestamp * w  # type: ignore
            self.squared_sum_timestamp = (
                timestamp * w * timestamp * w  # type: ignore
            )
    ...
    def insert(self, x, w, timestamp):
        self.n_samples += 1
        self.linear_sum_timestamp += timestamp * w
        self.squared_sum_timestamp += timestamp * w * timestamp * w
        for x_idx, x_val in x.items():
            self.linear_sum[x_idx] += x_val * w
            self.squared_sum[x_idx] += x_val * w * x_val * w

    def relevance_stamp(self, max_mc):
        if self.n_samples < 2 * max_mc:
            return self._mu_time
        return self._mu_time + self._sigma_time * self._quantile(
            float(max_mc) / (2 * self.n_samples)
        )
    ...
    def __iadd__(self, other: "CluStreamMicroCluster"):
        ...
        self.linear_sum_timestamp += other.linear_sum_timestamp
        self.squared_sum_timestamp += other.squared_sum_timestamp
        utils.skmultiflow_utils.add_dict_values(
            self.linear_sum, other.linear_sum, inplace=True
        )
        utils.skmultiflow_utils.add_dict_values(
            self.squared_sum, other.squared_sum, inplace=True
        )
        return self
